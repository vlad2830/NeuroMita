        ...
        public static void Update()
        {
            try
            {
                ...
                if (Camera.main != null && Camera.main.enabled && Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out RaycastHit hit))
                {
                    ...
                    if ( (objectName.Contains("Mita") || objectName.Contains("head)")) && objectViewTime[objectName] > 25f) EventsModded.LongWatching(objectName, objectViewTime[objectName]);
                    else if (objectViewTime[objectName] > 40f) EventsModded.LongWatching(objectName, objectViewTime[objectName]);
