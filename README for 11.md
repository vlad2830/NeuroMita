...
### 1) Мод
Мой мод идет в комплекте питон+промты+папкаКонвертации (их ставьте где угодно вместе) и также файлы мода (MitaAI.dll и assetbudle.test) непосредственно в папку mods, 
...
### 2) Генерация текста
Мод в плане генерации текста можно запустить в двух форматах.
   ...
   - Если у вас есть ключи напрямую (вы не в рф или у вас хорошо настроен впн), то как бы тоже варик, проверен опенроутер. Но, могут быть проблемы с форматами, так например напрямую с gemeni работать не получится. 
...
## 4) Генерация Голоса
Генерация голоса идет от тг бота силеро @silero_voice_bot или же также возмжно голос БМ от @CrazyMitaAIbot. В Силеро, чтобы был голос Миты нужен скорее всего какой-то прем, посмотрите там что доступно по HD голосам и кол-ву символов (моих 66к хватает за глаза)
...
Для юзания силеро используется telegram api, то есть ваш акк (желательно не основной) превращается в бота, в том плане что им можно управлять из кода. Сделал так со своим, но оцените риски сами) Либо спросите у остальных.
